...
static inline uint32_t arm_smc32_call(uint32_t function_id, uint32_t arg0, uint32_t arg1, uint32_t arg2)
{
    ...
    // FIXME: Whether to use SMC or HVC does not depend solely on whether or
    //        not we are in EL1, but on some more information that I don't know.
    // Compare QEMU with virtualization=on and virtualization=off.
    asm volatile(
        "smc #0\n"
        : "=r"(w0)
        : "r"(w0), "r"(w1), "r"(w2), "r"(w3)
