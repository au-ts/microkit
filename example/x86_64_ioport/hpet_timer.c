...
{
    uint64_t time = *(uint64_t *)(hpet_regs + MAIN_COUNTER_REG);
...
void init(void)
{
    volatile uint64_t *general_config_reg = (void *)hpet_regs + GENERAL_CONFIG_REG;
    *general_config_reg |= (1ul << ENABLE_CNF);

    timer_0 = (void *)hpet_regs + TIMERS_OFFSET;
    timer_0->config |= (1ul << TN_FSB_EN_CNF) | (1ul << TN_INT_ENB_CNF);
    timer_0->fsb_irr = ((0x0FEEllu << FIXED) << 32llu) | 0x000 | 0x30;

    __atomic_signal_fence(__ATOMIC_ACQ_REL);

    microkit_dbg_puts("timer PD initialised, current time is: ");
    puthex64(get_time());
    microkit_dbg_puts("\n");
    ...
    set_timeout(now + 1000000000);
}

void notified(microkit_channel ch)
{
    if (ch != IRQ_CH) {
        microkit_dbg_puts("non irq\n");
        return;
    }

    microkit_dbg_puts("got timeout, current time is: ");
    puthex64(get_time());
    microkit_dbg_puts("\n");
