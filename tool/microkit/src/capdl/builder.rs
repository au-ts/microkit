...
/// allow capability transfer.
const RIGHTS_EP_SEND: Rights = Rights {
    write: true,
    read: false,
    grant: false,
    grant_reply: false,
...
const RIGHTS_EP_RECEIVE: Rights = Rights {
    read: true,
    write: false,
    grant: false,
    grant_reply: false,
...
/// Minimal rights for a fault handler *of a TCB*.
/// Conditions for 'validFaultHandler()' in seL4 source:
/// We need 'send'/'capCanSend' and a grant or grantreply.
/// FIXME: why need a grant?
const RIGHTS_FAULT_HANDLER: Rights = Rights {
    read: false,
    write: true,
    grant: false,
    grant_reply: true,
...
const RIGHTS_NTFN_SIGNAL: Rights = Rights {
    read: false,
    write: true,
...
const RIGHTS_NTFN_RECEIVE: Rights = Rights {
    read: true,
    write: false,
    // Irrelevant for notifications, seL4 manual v13.0.0 pg11
    grant: false,
...
/// rights in this case.
const RIGHTS_SC_BIND: Rights = Rights {
    read: false,
    write: false,
    grant: false,
    grant_reply: false,
...
pub fn build_capdl_spec(
    ...
    for channel in system.channels.iter() {
        ...
        if channel.end_a.pp {
            ...
            let pd_a_ep_cap =
                capdl_util_make_endpoint_cap(pd_b_ep_id, RIGHTS_EP_SEND, pd_a_ep_badge);
            pd_a_shadow_cspace.insert_cap_into_microkit_cnode(
                &mut spec_container,
        ...
        if channel.end_b.pp {
            ...
            let pd_a_ep_id = pd_shadow_cspaces[&channel.end_a.pd]
                ...
                .expect("exists as needs_ep() is true");
            let pd_b_ep_cap =
                capdl_util_make_endpoint_cap(pd_a_ep_id, RIGHTS_EP_SEND, pd_b_ep_badge);
            pd_b_shadow_cspace.insert_cap_into_microkit_cnode(
                &mut spec_container,
