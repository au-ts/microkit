...
const PD_BASE_VSPACE_CAP: u64 = PD_BASE_PD_TCB_CAP + 64;
const PD_BASE_VM_TCB_CAP: u64 = PD_BASE_VSPACE_CAP + 64;
const PD_BASE_VCPU_CAP: u64 = PD_BASE_VM_TCB_CAP + 64;
const PD_BASE_IOPORT_CAP: u64 = PD_BASE_VCPU_CAP + 64;
// @kwinter: Is there a limit on the number of ioport caps?
...
pub fn build_capdl_spec(
    ...
    for (pd_global_idx, pd) in system.protection_domains.iter().enumerate() {
        if pd.child_pts {
            ...
            let mut cur_vaddr: u64 = kernel_config.pd_stack_bottom(pd.stack_size) as u64 - (table_data.len() as u64 * 2);
            table_metadata.base_addr = cur_vaddr;

            // And we need to get the vspace id
