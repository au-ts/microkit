...
use crate::{
    ...
    {TableMetadata, TopLevelPageTable, PGD, PUD},
};
    ...
    for (pd_global_idx, pd) in system.protection_domains.iter().enumerate() {
        ...
        if pd.page_table_copies.is_some() {
            ...
            for pt_copy in pd.page_table_copies.clone().unwrap().entries.iter() {
                let source_pd_index = *pd_name_to_id.get(&pt_copy.source_pd).unwrap();
                ...
                let pd_frame_metadata = pd_id_to_frame_metadata.get(&source_pd_index).unwrap();
