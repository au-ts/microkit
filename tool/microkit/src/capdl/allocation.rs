...
use crate::{
    capdl::{spec::capdl_obj_physical_size_bits, CapDLSpecContainer},
...
pub fn simulate_capdl_object_alloc_algorithm(
    ...
    for (_ut_orig_idx, ut) in untypeds_by_paddr.iter() {
                    ...
                    for size_bits in (0..=max_size_bits).rev() {
                        ...
                        if obj_id_range_maybe.is_some() {
                                ...
                                < obj_id_range_maybe.as_ref().unwrap().end
                            {
                                ...
                                // @billn revisit necessity after setvar region_paddr
