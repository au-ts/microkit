...
//
use crate::{capdl::{spec::{cap, object, Cap, NamedObject, Object, ObjectId}, CapDLSpec}, sel4::PageSize};

pub trait ArchMethods {
    fn vspace(pd_name: &str) -> NamedObject;
    fn map_page(spec: &mut CapDLSpec, pd_name: &str, vspace_id: ObjectId, frame_cap: Cap, frame_size: PageSize, vaddr: u64) -> Result<(), String> ;
}
...
fn map_intermediary_level_helper(spec: &mut CapDLSpec, pd_name: &str, next_level_name_prefix: &str, cur_level_obj_id: ObjectId, cur_level: u8, cur_level_slot: u64) -> ObjectId {
    match spec.get_root_object_mut(cur_level_obj_id) {
        Some(vspace_obj) => {
            match &mut vspace_obj.object {
                Object::PageTable(page_table_object) => {
                    match page_table_object.slots.iter().find(|cte| cte.0 == cur_level_slot as usize) {
                        Some(cte_unwrapped) => {
                ...
                Object::PageTable(page_table_object) => {
                    match page_table_object.slots.iter().find(|cte| cte.0 == cur_level_slot as usize) {
                        ...
                        None => {
                            ...
                            page_table_object.slots.push((cur_level_slot as usize, next_level_cap));
                        },
                    }
                },
                _ => todo!(),
...
fn insert_frame_cap_into_level(spec: &mut CapDLSpec, last_level_paging_obj_id: ObjectId, last_level_slot: u64, frame_cap: Cap) {
    match spec.get_root_object_mut(last_level_paging_obj_id) {
        Some(last_level_paging_obj) => {
            match &mut last_level_paging_obj.object {
                Object::PageTable(last_level_paging_inner_obj) => {
                    last_level_paging_inner_obj.slots.push((last_level_slot as usize, frame_cap));
                },
                _ => todo!(),
            }
        },
    ...
    fn map_page(spec: &mut CapDLSpec, pd_name: &str, vspace_id: ObjectId, frame_cap: Cap, frame_size: PageSize, vaddr: u64) -> Result<(), String> {
        match &frame_cap {
            Cap::Frame(_) => {
                ...
                let pdpt_obj_id: ObjectId = map_intermediary_level_helper(spec, pd_name, "pdpt", vspace_id, 0, pml4_slot);
                let pd_obj_id: ObjectId = map_intermediary_level_helper(spec, pd_name, "pd", pdpt_obj_id, 1, pdpt_slot);
                let pt_obj_id: ObjectId = map_intermediary_level_helper(spec, pd_name, "pt", pd_obj_id, 2, pd_slot);
