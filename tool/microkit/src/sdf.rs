...
impl IOAddressSpace {
    fn from_xml(
        ...
        // In microkit enforcing uniqueness for domain ids is hugely simplifying and enforces better designs.
        // If we were to allow shared domain ids, we would have to ensure that whenever two devices share a domain
        // that they do not have any IOVA to physyical address mappings that conflict, and that they share the same
        // IOPT objects, this because the source identifier aka the PCI BDF is not used in all caches.
        // See section 6.2.1 and table 17 in the intel VT-D manual.
        // https://www.google.com/url?sa=t&source=web&rct=j&opi=89978449&url=https://cdrdv2-public.intel.com/671081/vt-directed-io-spec.pdf&ved=2ahUKEwiN_YawksWVAxUDmFYBHfzzCEQQFnoECA0QAQ&usg=AOvVaw3fO9wnba58SqsP2HNKwdfu
        // We make the user specify this information though since at runtime the selected domain_ids might be
        // reserved by seL4 during its boot and therefore unavailable. This forces the user of microkit to select a new
        // domain id.
