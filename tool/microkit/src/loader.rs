...
pub mod aarch64 {
    ...
    pub fn block_descriptor(level: usize, addr: u64, mem_attr: MemAttr) -> u64 {
        ...
        // bit[7:6] contains AP[2:1], which we set as 0b00 for read/write access:
        ...
        let lower_attributes: u64 =
            (1 << 10) | (0b00 << 6) | (shareability << 8) | (mem_attr.value() << 2);
...
impl<'a> Loader<'a> {
    ...
    fn aarch64_setup_pagetables(
        ...
        let memattr_normal = if hypervisor {
            MemAttr::Stage2 { mem_attr: aarch64::s2_mem_attr::NORMAL_INNER_WBC_OUTER_WBC }
        } else {
            MemAttr::Stage1 { attr_index: aarch64::s1_mair_attr_index::MT_NORMAL }
        };
        #[rustfmt::skip]
        let memattr_device = if hypervisor {
            MemAttr::Stage2 { mem_attr: aarch64::s2_mem_attr::DEVICE_nGnRnE }
        } else {
            MemAttr::Stage1 { attr_index: aarch64::s1_mair_attr_index::MT_DEVICE_nGnRnE }
        };
