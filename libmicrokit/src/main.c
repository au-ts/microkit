...
static seL4_MessageInfo_t x86_vcpu_resume(seL4_Word *badge)
{
    ...
    x64_sys_send_recv(seL4_SysVMEnter, 0, badge, 0, &is_fault,
                      &microkit_x86_vcpu_state.rip, &microkit_x86_vcpu_state.prim_proc_ctl, &microkit_x86_vcpu_state.irq_info, &fault_reason,
                      0);
    /* We want to follow the documented kernel behaviour where these 4 values are always populated in the message registers.
     * However, due to `setMR()`'s behaviour in include/object/tcb.h of the kernel, these 4 values will only be set in
     * CPU registers. So we need to copy them to the IPC buffer to conform to the documented kernel behaviour.
     *
     * The other reason why we want to manage these values is so that the user does not have to worry about saving/updating them
     * when servicing a notification. */
    ...
    /* The 4th value (the exit reason) is only valid when fault is true. */

    /* Create a dummy msgInfo so that we can call `fault()`, as on x86 a VMExit is not an IPC as on other architectures. */
    if (is_fault) {
